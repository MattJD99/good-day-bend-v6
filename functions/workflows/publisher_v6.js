...
const runTrendBlogV6 = require('./trend_blog_v6');
...
async function runPublisherV6(targetDateInput) {
    ...
    if (strategyData.headliners && strategyData.headliners.length > 0) {
        for (const title of strategyData.headliners) {
            ...
            if (event && (event.image || event.imageUrl)) {
                ...
                // Ensure it's not a stock fallback itself and looks like a URL
                // Also ignore "placehold.co" if we want to avoid generic placeholders
    ...
    console.log("✍️ Node 3: Writing Styled Article...");
    ...
    * **CRITICAL**: Use specific classes or inline styles to enforce the "Massive" look.
    ...
    const socialDraftHtml = `
        ...
        <body class="min-h-screen flex items-center justify-center py-12 px-4 pb-32">
             ...
             <!-- Status Header -->
            <div class="bg-[#0d1b12] text-[#13ec5b] px-6 py-4 text-sm font-bold tracking-widest uppercase flex justify-between items-center sticky top-0 z-40 shadow-md w-full fixed left-0 right-0">
                <span>Good Day Bend // Social Draft</span>
                <span>${getTodayStr()}</span>
            </div>

            <!-- Instagram Phone Mockup -->
            <div class="bg-white w-full max-w-[400px] rounded-[3rem] border-[8px] border-[#0d1b12] shadow-2xl overflow-hidden relative mt-16">
                <!-- Status Bar -->
            ...
            <!-- Sticky Action Bar -->
            <div style="position: fixed; bottom: 0; left: 0; right: 0; background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); padding: 20px; display: flex; justify-content: center; gap: 20px; box-shadow: 0 -4px 20px rgba(0,0,0,0.1); z-index: 50; border-top: 1px solid #e2e8f0;">
                 <a href="${FUNCTION_URL}?type=review_action&action=keep&draftId=${draftId}" 
                   class="px-6 py-3 rounded-full text-sm font-bold text-gray-500 hover:text-gray-900 border border-gray-200 hover:bg-gray-50 transition">
                   📂 Keep as Draft
                </a>
