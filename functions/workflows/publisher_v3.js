...
const modelImagePro = getSegmentedModel('DAILY_UPDATE', CONFIG.MODEL_IMAGE_PRO); // Note: Used for text logic or image prompt refinement if needed

async function runPublisher(targetDateInput) {
    console.log("💎 Opal Publisher V3 Starting...");
    ...
    SEO & AEO UPGRADES:
    1.  **Alt Text**: Ensure any images or icons have descriptive alt text (e.g., alt="Live music at Volcanic Theatre Pub").
    2.  **Internal Links**: Mention "Check our full calendar" linking to /calendar.html at least once.
    3.  **Local Authority**: When mentioning venues, use their full recognizable names to help local search entity recognition.
    4.  **Trusted Sources**: Since this is a daily update, try to validly link to one of these authoritative sources if relevant: ${sourcesText}.
    ...
    if (bestEventImage) {
        ...
        // We use the 'vibe' AND the specific 'headliners' to make a composite scene
        const headlinesText = strategyData.headliners ? strategyData.headliners.join(", ") : "Local Events";

        const imagePrompt = `
        A photorealistic, high-quality hero image for a blog post about today's vibe in Bend, Oregon: "${strategyData.vibe}".

        Visual Elements to Include:
        - Hints of the following activities: ${headlinesText}
        - Setting: Outdoors in Bend, Oregon (mountains or high desert trees in background).
        - Lighting: Golden hour or moody atmospheric lighting.
        - Style: Editorial photography, depth of field, f/1.8.
        - NO TEXT overlay.
        `;
