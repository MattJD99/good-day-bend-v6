...
"""
    ...
    # 7. Create Draft Previews
    print("\n💾 Creating draft previews...")
    
    draft_id = f"draft-{int(datetime.now().timestamp())}"
    
    # Upload web version
    blog_url = await upload_draft_html(f"blog-draft-{draft_id}.html", f"<html><body><img src='{image_url}' style='width:100%'>{article.html}</body></html>")
    
    # Upload email version
    email_url = await upload_draft_html(f"email-draft-{draft_id}.html", email_html)
    
    # Create social text file
    social_text = f"CAPTION:\n{social.caption}\n\nSMS:\n{social.sms}"
    social_url = await upload_draft_html(f"social-draft-{draft_id}.html", social_text)  # Reusing html uploader for simple text view
    
    print(f"✅ Blog draft: {blog_url}")
    print(f"✅ Email draft: {email_url}")
    print(f"✅ Social draft: {social_url}")
    
    # Step 7: Save draft metadata to Firestore
    await asyncio.to_thread(
    ...
    # Step 8: Send Approval Email via GHL
    print("\n📧 Sending Approval Email...")
    try:
        ADMIN_EMAIL = "mdesautel@gmail.com"
        FUNCTION_URL = "https://us-central1-good-day-bend-v6.cloudfunctions.net/handleRequest"
        
        approve_link = f"{FUNCTION_URL}?type=review_action&action=approve&draftId={draft_id}"
        keep_link = f"{FUNCTION_URL}?type=review_action&action=keep&draftId={draft_id}"
        
        email_subject = f"✏️ Daily Update Draft: {article.title}"
        email_body = f"""
        <div style="font-family: 'Outfit', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: #0d1b12; color: #13ec5b; padding: 15px 20px; border-radius: 12px 12px 0 0; text-align: center;">
                <h2 style="margin: 0;">📝 Daily Update Draft Ready</h2>
            </div>
            <div style="background: white; padding: 25px; border: 1px solid #e2e8f0; border-top: none;">
                <h3 style="color: #0d1b12; font-size: 1.4rem;">{article.title}</h3>
                <p style="color: #64748b;">Vibe: {vibe_data.vibe}</p>
                <img src="{image_url}" style="width: 100%; border-radius: 8px; margin: 15px 0;" />
                <p><strong>Preview Links:</strong></p>
                <ul>
                    <li><a href="{blog_url}">Blog Preview</a></li>
                    <li><a href="{email_url}">Email Preview</a></li>
                    <li><a href="{social_url}">Social Preview</a></li>
                </ul>
            </div>
            <div style="text-align: center; padding: 20px; background: #f8fafc; border-radius: 0 0 12px 12px;">
                <a href="{approve_link}" style="background: #13ec5b; color: #0d1b12; padding: 14px 32px; border-radius: 50px; text-decoration: none; font-weight: bold; font-size: 1.1rem; display: inline-block; margin: 5px;">🚀 Approve & Publish</a>
                <a href="{keep_link}" style="background: white; color: #64748b; padding: 14px 32px; border-radius: 50px; text-decoration: none; font-weight: bold; border: 1px solid #e2e8f0; display: inline-block; margin: 5px;">📂 Keep Draft</a>
            </div>
        </div>
        """
        
        ghl = GHLClient()
        contact_id = ghl.upsert_contact(ADMIN_EMAIL, first_name="Matt", last_name="Admin")
        if contact_id:
...
def create_blog_draft_html(
    ...
    # TODO: Update with actual function URL
    function_url = "https://main-nfvkqznekq-uc.a.run.app"
